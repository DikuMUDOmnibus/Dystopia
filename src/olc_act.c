...
bool redit_rlist(CHAR_DATA * ch, char *argument)
{
  ...
  for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
  {
    ...
    {
      found = TRUE;
      cprintf(buf, "[%6d] %-48.48s#n [%-10s]", vnum, pRoom->name, flag_string(sector_flags, pRoom->sector_type));
      strcat(buf1, buf);
      cprintf(buf, " [%s]\n\r", flag_string(room_flags, pRoom->room_flags));
      strcat(buf1, buf);

      /* after 50 rooms, send and flush */
      if (++i >= 50)
      {
        delay_message(buf1, ch, delay);
        i = 0;
        delay += 2 * PULSE_PER_SECOND;
        buf1[0] = '\0';
      }
    }
  }

  /* got anything pending ? */
  if (buf1[0] != '\0')
    delay_message(buf1, ch, delay);
...
bool redit_spell(CHAR_DATA * ch, char *argument)
{
  ...
  {
    pIter = AllocIterator(pRoom->roomtext);
    while ((rt = (ROOMTEXT_DATA *) NextInList(pIter)) != NULL)
    {
  ...
  if (!str_cmp(keyword, "delete"))
  {
    ...
    DetachFromList(rt, pRoom->roomtext);
  ...
  else if (!str_cmp(keyword, "create"))
  {
    ...
    AttachToList(rt, pRoom->roomtext);
...
bool redit_mlist(CHAR_DATA * ch, char *argument)
{
  ...
  sprintf(buf1, "#u#9  VNUM   Mobile Name              Level  Might  Flags#n\n\r");
  for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
  {
    if ((pMobIndex = get_mob_index(vnum)))
    {
      ...
      {
        found = TRUE;
        sprintf(buf, "[%5d]  %-25.24s %4d   %4d  %s\n\r",
          pMobIndex->vnum, capitalize(pMobIndex->short_descr),
          pMobIndex->level, getMobMight(pMobIndex), flag_string(act_flags, pMobIndex->act));
        strcat(buf1, buf);
      }
    }
  }
...
bool redit_olist(CHAR_DATA * ch, char *argument)
{
  ...
  for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
  {
    if ((pObjIndex = get_obj_index(vnum)))
    {
      ...
      {
        found = TRUE;
        cprintf(buf, "[%5d] %-17.16s", pObjIndex->vnum, capitalize(pObjIndex->short_descr));
        strcat(buf1, buf);
        if (++col % 3 == 0)
          strcat(buf1, "\n\r");
      }
...
bool aedit_file(CHAR_DATA * ch, char *argument)
{
  ...
  {
    send_to_char("You cannot change the filename of this area.\n\r", ch);
    return FALSE;
  }
  ...
  if (length > 24)
  {
    send_to_char("No more than 24 characters allowed.\n\r", ch);
    return FALSE;
  }
...
bool aedit_vnum(CHAR_DATA *ch, char *argument)
{
  ...
  if (ch->level < MAX_LEVEL && pArea->uvnum != 0)
  {
    send_to_char("You can only change the vnum range of an area once.\n\r", ch);
    return FALSE;
  }
...
bool redit_show(CHAR_DATA * ch, char *argument)
{
  ...
  if (SizeOfList(pRoom->extra_descr) != 0)
  {
    EXTRA_DESCR_DATA *ed;
  ...
  if (SizeOfList(pRoom->roomtext) > 0)
  {
    ROOMTEXT_DATA *rt = NULL;
...
bool redit_ed(CHAR_DATA * ch, char *argument)
{
  ...
  if (!str_cmp(command, "add"))
  {
    ...
    ed->vnum = 0;
    AttachToList(ed, pRoom->extra_descr);
...
bool redit_mreset(CHAR_DATA * ch, char *argument)
{
  ...
  char_to_room(newmob, pRoom, TRUE);
...
bool redit_oreset(CHAR_DATA * ch, char *argument)
{
  ...
  if (arg1[0] == '\0' || !is_number(arg1))
  {
    ...
    send_to_char("        -<mob_name> <wear-loc> = into mob\n\r", ch);
    return FALSE;
  }
  ...
  if ((to_mob = get_char_room(ch, arg2)) != NULL)
  {
     ...
     */
    if (wearloc != WEAR_NONE && get_eq_char(to_mob, wearloc))
    {
    ...
    olevel = URANGE(0, to_mob->level - 2, LEVEL_AVATAR);
  ...
  switch (obj->item_type)
  {
    ...
    case ITEM_DRINK_CON:
      sprintf(buf,
              "[v0] Liquid Total: [%d]\n\r"
              "[v1] Liquid Left:  [%d]\n\r"
              "[v2] Liquid:       %s\n\r" "[v3] Poisoned:     %s\n\r", obj->value[0], obj->value[1], flag_string(liquid_flags, obj->value[2]), obj->value[3] != 0 ? "Yes" : "No");
      send_to_char(buf, ch);
      break;
...
bool oedit_show(CHAR_DATA * ch, char *argument)
{
  ...
  if (SizeOfList(pObj->extra_descr) != 0)
  {
    EXTRA_DESCR_DATA *ed;

    send_to_char("Ex desc kwd: ", ch);

    pIter = AllocIterator(pObj->extra_descr);
    while ((ed = (EXTRA_DESCR_DATA *) NextInList(pIter)) != NULL)
    {
      send_to_char("[", ch);
  ...
  pIter = AllocIterator(pObj->affected);
...
bool oedit_addaffect(CHAR_DATA * ch, char *argument)
{
  ...
  AttachToList(pAf, pObj->affected);
...
bool oedit_create(CHAR_DATA * ch, char *argument)
{
  ...
  AttachToList(pObj, obj_index_hash[iHash]);
  ch->desc->pEdit = (void *) pObj;
...
bool oedit_ed(CHAR_DATA * ch, char *argument)
{
  ...
  if (!str_cmp(command, "add"))
  {
    ...
    AttachToList(ed, pObj->extra_descr);
  ...
  if (!str_cmp(command, "delete"))
  {
    ...
    DetachFromList(ed, pObj->extra_descr);
...
bool medit_show(CHAR_DATA * ch, char *argument)
{
  ...
  printf_to_char(ch, "v0: Toughness         [%4d]\n\r", pMob->toughness);
  printf_to_char(ch, "v1: Damage Modifier   [%4d]\n\r", pMob->dam_modifier);
  printf_to_char(ch, "v2: Extra Attacks     [%4d]\n\r", pMob->extra_attack);
  printf_to_char(ch, "v3: Dodge Bonus       [%4d]\n\r", pMob->extra_dodge);
  printf_to_char(ch, "v4: Parry Bonus       [%4d]\n\r", pMob->extra_parry);

  switch (pMob->natural_attack)
  {
    default:
      sprintf(buf, "Weapon     : [Hit]\n\r");
      break;
    case 1:
      sprintf(buf, "Weapon     : [Slice]\n\r");
      break;
    case 2:
      sprintf(buf, "Weapon     : [Stab]\n\r");
      break;
    case 3:
      sprintf(buf, "Weapon     : [Slash]\n\r");
      break;
    case 4:
      sprintf(buf, "Weapon     : [Whip]\n\r");
      break;
    case 5:
      sprintf(buf, "Weapon     : [Claw]\n\r");
      break;
    case 6:
      sprintf(buf, "Weapon     : [Blast]\n\r");
      break;
    case 7:
      sprintf(buf, "Weapon     : [Pound]\n\r");
      break;
    case 8:
      sprintf(buf, "Weapon     : [Crush]\n\r");
      break;
    case 9:
      sprintf(buf, "Weapon     : [Grep]\n\r");
      break;
    case 10:
      sprintf(buf, "Weapon     : [Bite]\n\r");
      break;
    case 11:
      sprintf(buf, "Weapon     : [Pierce]\n\r");
      break;
    case 12:
      sprintf(buf, "Weapon     : [Suck]\n\r");
      break;
  }
  send_to_char(buf, ch);
  ...
  if (pMob->death_fun)
  {
    sprintf(buf, "Death fun:    [%s]\n\r", death_string(pMob->death_fun));
    send_to_char(buf, ch);
  }
...
bool medit_create(CHAR_DATA * ch, char *argument)
{
  ...
  AttachToList(pMob, mob_index_hash[iHash]);
  ch->desc->pEdit = (void *) pMob;
...
bool medit_death(CHAR_DATA *ch, char *argument)
{
  MOB_INDEX_DATA *pMob;
  ...
  if (argument[0] == '\0')
  {
    send_to_char("Syntax:  death [death function]\n\r", ch);
    send_to_char("        'death none' to remove\n\r", ch);
    return FALSE;
  }
  if (!str_cmp(argument, "none"))
  {
    pMob->death_fun = NULL;
    send_to_char("Death removed.\n\r", ch);
    return TRUE;
  }
  if (death_lookup(argument))
  {
    pMob->death_fun = death_lookup(argument);
    send_to_char("Death set.\n\r", ch);
    return TRUE;
  }
  send_to_char("MEdit: No such death function.\n\r", ch);
  return FALSE;
}
...
bool medit_value0(CHAR_DATA * ch, char *argument)
{
  ...
  {
    send_to_char("Range is from -100 to 100.\n\r", ch);
    return FALSE;
  }
...
bool medit_value1(CHAR_DATA * ch, char *argument)
{
  ...
  if (atoi(argument) < -50 || atoi(argument) > 200)
  {
    send_to_char("Range is from -50 to 200.\n\r", ch);
    return FALSE;
  }
...
bool medit_value2(CHAR_DATA * ch, char *argument)
{
  ...
  if (atoi(argument) < 0 || atoi(argument) > 15)
  {
    send_to_char("Range is from 0 to 15.\n\r", ch);
    return FALSE;
  }
...
bool medit_value3(CHAR_DATA * ch, char *argument)
{
  MOB_INDEX_DATA *pMob;
  ...
  if (argument[0] == '\0')
  {
    send_to_char("Syntax: v3 [number]\n\r", ch);
    return FALSE;
  }

  if (atoi(argument) < -50 || atoi(argument) > 50)
  {
    send_to_char("Range is from -50 to 50.\n\r", ch);
    return FALSE;
  }

  pMob->extra_dodge = atoi(argument);

  send_to_char("v3 set.\n\r", ch);
  return TRUE;
}

bool medit_value4(CHAR_DATA * ch, char *argument)
{
  MOB_INDEX_DATA *pMob;
  ...
  pMob->extra_parry = atoi(argument);
